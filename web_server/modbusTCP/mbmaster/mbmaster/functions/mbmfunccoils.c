...
/* ----------------------- Defines ------------------------------------------*/
#define NBYTES_FOR_NINPUTS( usNInputs ) ( UBYTE )( ( ( usNInputs ) + 7U ) / 8U )
...
#define MBM_PDU_FUNC_RD_COILS_RESPSIZE( usNCoils ) ( UBYTE )( NBYTES_FOR_NINPUTS( usNCoils ) + 1 + 1 )
...
#define NBYTES_FOR_NCOILS( usNCoils ) ( UBYTE )( ( ( usNCoils ) + 7U ) / 8U )
                ...
                if( ( NULL != arubBufferOut ) && ( usNCoils > 0 ) && ( usNCoils <= MBM_PDU_FUNC_RD_COILSCNT_MAX ) )
                {
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     "[IDX=" MBP_FORMAT_USHORT "] Read coils (start=" MBP_FORMAT_USHORT
                                     ", length=" MBP_FORMAT_USHORT ").\n",
                                     ( USHORT )pxIntHdl->ubIdx, usCoilStartAddress, ( USHORT )usNCoils );
                    }
#endif
                    ...
                    pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] =
                        ( UBYTE )( usCoilStartAddress & 0x00FFU );
                    pxIntHdl->usFrameMBPDULength++;
            ...
            case MBM_STATE_DISASSEMBLE:
                *peState = MBM_STATE_DONE;
                    ...
                    ( MBM_FUNC_WR_SINGLE_COIL == pxIntHdl->pubFrameMBPDUBuffer[MB_PDU_FUNC_OFF] ) )
                {
                     ...
                     * rules.
                     */
                    usOutputAddressResp = ( USHORT )(
                        ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_SINGLE_COIL_RESP_ADDR_OFF] << 8U );
                    usOutputAddressResp |= ( USHORT )(
                        ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_SINGLE_COIL_RESP_ADDR_OFF + 1] );
                    usOutputValueResp = ( USHORT )(
                        ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_SINGLE_COIL_RESP_VALUE_OFF] << 8U );
                    usOutputValueResp |= ( USHORT )(
                        ( USHORT )pxIntHdl->pubFrameMBPDUBuffer[MBM_FUNC_WR_SINGLE_COIL_RESP_VALUE_OFF + 1] );
...
void
vMBMWriteCoilsPolled( xMBMHandle xHdl, UCHAR ucSlaveAddress, USHORT usCoilStartAddress, USHORT usNCoils,
        ...
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                if( ( NULL != pubCoilInPtr ) && ( usNCoils <= MBM_FUNC_WR_COILS_COILSCNT_MAX ) )
                {
                    ...
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     "[IDX=" MBP_FORMAT_USHORT "] Write coils (start=" MBP_FORMAT_USHORT
                                     ", length=" MBP_FORMAT_USHORT ").\n",
